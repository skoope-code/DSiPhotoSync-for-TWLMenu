...
static void redraw_menu(int sel, int n_unconv, int n_conv) {
    ...
    const char *labels[MENU_ITEMS] = { l0, l1, "Use with custom theme", "Settings" };
...
static void draw_variant_list(const char *title, int nrows, const int *vk,
    ...
    for (int r = 0; r < nrows; r++) {
        ...
        if (vk[r] == 2) {
            ...
            // and the label are vertically centered on each other.
            y += 2 * (ROW_H + 2);
            if (is_sel) fb_rect(LIST_X - 2, y - 1, ROW_W, ROW_H, COL_HILITE);
    ...
    while (screen != SCREEN_EXIT) {
        ...
        if (screen == SCREEN_MENU) {
            ...
            if ((down & KEY_A) && ln > 0) {
                ...
                if (cur_theme == THEME_DEFAULT_ROW) {
                    ...
                    draw_theme_list(list, ln, theme_sel, theme_top, theme_page,
                                    theme_applied_idx);
                    // Status message sits below the "# of #" counter line.
                    ...
                    // Only now -- when control is about to return to the user --
                    // swap to "Theme applied." so it appears exactly when the
                    // screen becomes interactive again.
                    fb_rect(LIST_X, dmsg_y - 2, ROW_W, 12, COL_BG);
            ...
            if (down & KEY_A) {
                ...
                // isn't interactive yet -- then flip to the result exactly when
                // control returns to the user, so the label always matches what
